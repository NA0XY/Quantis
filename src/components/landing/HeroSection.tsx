...
export function HeroSection() {
  ...
  return (
    <section
      ref={container}
      className="relative min-h-screen flex flex-col items-center justify-start pt-12 px-4 bg-sky overflow-hidden"
    >
      <div className="absolute inset-0 z-0 pointer-events-none opacity-[0.08]">
        <div
          className="absolute inset-0"
          style={{ backgroundImage: "radial-gradient(#111 2px, transparent 2px)", backgroundSize: "40px 40px" }}
        />
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center flex flex-col items-center">
        <Badge className="hero-badge mb-4 bg-chalk border-4 border-ink text-ink shadow-[4px_4px_0_#111] uppercase tracking-[0.2em] font-black px-4 py-1">
          AI Generator + Coach Online / v0.42.0
        </Badge>
        ...
        <h1 className="hero-title text-6xl md:text-8xl font-black text-ink leading-[0.85] tracking-tighter mb-6 max-w-4xl uppercase overflow-hidden">
          <span className="inline-block">Your trading</span>
          <br />
          <span className="inline-block text-primary [text-shadow:6px_6px_0_#000]">strategy</span>
          <br />
          <span className="inline-block underline decoration-[12px] decoration-ink underline-offset-[-12px]">
            Coached by AI
          </span>
        </h1>

        <p className="hero-desc text-lg md:text-xl text-ink font-bold max-w-2xl mx-auto mb-6 leading-snug">
          Describe the idea in plain English, generate runnable Quantis Python, scan 41 live markets,
          and only ship the version you trust.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 mb-12">
          <Link href="/signup" className="hero-cta">
            <Button
              variant="primary"
              size="lg"
              className="shadow-[8px_8px_0_#000] active:translate-x-1 active:translate-y-1 active:shadow-[4px_4px_0_#000] text-xl uppercase font-black py-8 px-12 border-4 group"
            >
              Start Building
          ...
          <Link href="/leaderboard" className="hero-cta">
            <Button
              variant="secondary"
              size="lg"
              className="shadow-[8px_8px_0_#000] text-xl uppercase font-black py-8 px-12 border-4 bg-chalk hover:bg-chalk/80"
            >
              Leaderboard
            </Button>
          </Link>
          ...
          <div className="w-full bg-chalk border-t-8 border-l-8 border-r-8 border-ink rounded-t-[40px] shadow-[20px_0_0_#000] overflow-hidden">
            <div className="w-full h-12 bg-ink flex items-center justify-between px-8">
              ...
              <div className="text-chalk/40 font-mono text-[10px] uppercase font-black tracking-[0.4em]">
                Live Workspace / ai_generated_edge.py
              </div>
            ...
            <div className="p-8 grid grid-cols-[1fr_2fr] gap-8 bg-sky/20">
              <div className="space-y-4">
                <div className="h-20 bg-primary border-4 border-ink shadow-[8px_8px_0_#000] p-4 flex flex-col justify-between">
                  <span className="text-[10px] font-black uppercase text-ink/40">AI Draft Status</span>
                  <span className="text-2xl font-black text-ink">Runnable</span>
                </div>
                <div className="h-20 bg-chalk border-4 border-ink shadow-[8px_8px_0_#000] p-4 flex flex-col justify-between">
                  <span className="text-[10px] font-black uppercase text-ink/40">Scan Coverage</span>
                  <span className="text-2xl font-black text-[#00c853]">41 Markets</span>
                </div>
                <div className="flex space-x-2">
                  <div className="flex-1 h-8 bg-ink text-chalk text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center">
                    AI Coach
                  </div>
                  <div className="flex-1 h-8 bg-ink/20 border-2 border-ink text-ink text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center">
                    5 Min Cron
                  </div>
                </div>
              </div>
              ...
              <div className="relative h-44 bg-[#0a0a0a] border-4 border-ink shadow-[8px_8px_0_#000] overflow-hidden">
                <div className="absolute inset-0 flex items-end px-4 pb-4 space-x-2">
                  {[28, 44, 40, 55, 62, 74, 68, 92, 88, 100].map((height, i) => (
                    <div
                      key={i}
                ...
                <div className="absolute top-4 left-4 font-mono text-[10px] text-primary font-black uppercase">
                  AI Analyze - Best Market: BTCUSDT
                </div>
              </div>
            </div>
